"""
APP: API for playing the game from the browser
"""
...
from api.game_func import (init_board, shift_down, shift_left, shift_right,
                           shift_up)
...
def down():
    ...
    response = jsonify({"board": board.tolist(), "points": int(points)})
    response.status = 200
    response.headers.add("Access-Control-Allow-Origin", "*")
...
def get_model_sugestion():
    ...
    board_tensor = torch.tensor(
        np.array(board).flatten().tolist(), dtype=torch.float32
    ).unsqueeze(dim=0)
    q_values = model(board_tensor)[0].tolist()

    response = jsonify({"q_values": q_values})
    response.status = 200
    response.headers.add("Access-Control-Allow-Origin", "*")
