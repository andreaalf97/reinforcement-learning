...
def mean_random_moves(max_moves_allowed: int):
    return (
        50 * (1 - math.pow(math.e, -(max_moves_allowed - 110) / 50)) + 110
        if max_moves_allowed > 110
        else max_moves_allowed
    )
...
def init_networks(args):
    ...
    if not args.conv:
        main_network = FFN(16, 4, hidden_size=args.hidden_size).to(
            "cuda" if args.cuda else "cpu"
        )
        target_network = FFN(16, 4, hidden_size=args.hidden_size).to(
            "cuda" if args.cuda else "cpu"
        )
    else:
        main_network = ConvBrain((4, 4), 4, hidden_size=args.hidden_size).to(
            "cuda" if args.cuda else "cpu"
        )
        target_network = ConvBrain((4, 4), 4, hidden_size=args.hidden_size).to(
            "cuda" if args.cuda else "cpu"
        )

    if args.resume_from != "":
        assert os.path.exists(
            f"{args.store_run_at}/{args.resume_from}"
        ), f"Folder `{args.store_run_at}/{args.resume_from}` does not exist"
        for _, _, file_names in os.walk(f"{args.store_run_at}/{args.resume_from}"):
            ...
            if "model.pt" in all_models:
                main_network.load_state_dict(
                    torch.load(f"{args.store_run_at}/{args.resume_from}/model.pt")
                )
            else:
                main_network.load_state_dict(
                    torch.load(
                        f"{args.store_run_at}/{args.resume_from}/{all_models[-1]}"
                    )
                )
...
def train_model(
    ...
    losses: list,
    args,
    action_order=["right", "left", "up", "down"],
    ...
    training_sample = random.sample(
        replay_memory.get_memory(),
        args.n_samples_to_train_on
        if args.n_samples_to_train_on <= len(replay_memory)
        else len(replay_memory),
    )
    ...
    with torch.no_grad():
        current_states = torch.tensor(
            [i[0].tolist() for i in training_sample], dtype=torch.float32
        ).to("cuda" if args.cuda else "cpu")
        current_qs = main_network(current_states)
        new_states = torch.tensor(
            [i[2].tolist() for i in training_sample], dtype=torch.float32
        ).to("cuda" if args.cuda else "cpu")
    ...
    X, Y = [], []
    for index, (current_state, action_name, new_state, reward, done) in enumerate(
        training_sample
    ):
        if done:
            target_q = reward
        ...
        current_qs[index][action_index] = (1 - args.learning_rate) * current_qs[index][
            action_index
        ] + args.learning_rate * target_q

        X.append(
            torch.tensor(current_state, dtype=torch.float32).to(
                "cuda" if args.cuda else "cpu"
            )
        )
...
def main(args):
    ...
    now = datetime.now()
    base_folder_name = (
        f"{now.year:04d}_{now.month:02d}_{now.day:02d}_{now.hour:02d}_{now.minute:02d}"
    )
    ...
    for episode_number in range(args.episodes):
        ...
        while not done:
            ...
            if random.random() < current_epsilon:
                ...
                action_name = random.choice(
                    list([k for k in game.actions.keys() if k in available_actions])
                )
            else:
                n_best_actions += 1
                with torch.no_grad():
                    ...
                    missing_moves = [
                        k for k in game.actions.keys() if k not in available_actions
                    ]
                    missing_moves_indices = [
                        i
                        for i, k in enumerate(game.actions.keys())
                        if k in missing_moves
                    ]
            ...
            replay_memory.append((current_state, action_name, new_state, reward, done))

            if steps % args.update_main_network_every == 0:
                if args.log_training_events:
                    logger.warning("[M] Updating MAIN network")
                losses = train_model(
                    main_network, replay_memory, target_network, losses, args
                )
            ...
            if (episode_number % int(args.episodes / 5) == 0) and (episode_number != 0):
                if not args.no_store:
                    if not os.path.exists(
                        f"{args.store_run_at}/{base_folder_name}/checkpoint_{episode_number:04d}.pt"
                    ):
                        logger.warning(
                            f"Storing checkpoint model at {args.store_run_at}/{base_folder_name}/checkpoint_{episode_number:04d}.pt"
                        )
                        torch.save(
        ...
        if episode_number % int(args.episodes / 10) == 0:
            logger.info(
                f"[{episode_number}] Episode completed with epsilon {current_epsilon:.3f}"
            )
            logger.info(
                f"[{episode_number}] Avg reward: --[{sum([int(i) for i in rewards[-10:]])/10:.2f}]--"
            )
            logger.info(f"[{episode_number}] Memory size: {len(replay_memory)}")
            logger.info(f"[{episode_number}] Max moves: {max_moves}")
            logger.info(
                f"[{episode_number}] Avg moves: --[{sum([int(i) for i in episodes_total_steps[-10:]])/10:.2f}]--"
            )
            logger.info(
                f"[{episode_number}] Taken the best action {n_best_actions/(n_best_actions+n_random_actions)*100:.2f}% of the time"
            )
            logger.info(f"---------")
    ...
    if not args.no_store:
        logger.info(
            f"Storing run parameters at {args.store_run_at}/{base_folder_name}/run_info.json"
        )
        with open(f"{args.store_run_at}/{base_folder_name}/run_info.json", "w") as fp:
            json.dump(run_info, fp)
        logger.info(
            f"Storing trained model at {args.store_run_at}/{base_folder_name}/model.pt"
        )
        torch.save(
            target_network.state_dict(),
            f"{args.store_run_at}/{base_folder_name}/model.pt",
        )
        logger.info(
            f"Storing run last memory at {args.store_run_at}/{base_folder_name}/memory.pkl"
        )
    ...
    # )
    ax.set_title(
        f"Reward over episodes. [MAX {max([int(i) for i in run_info['rewards']])}]"
    )

    target_train_indices = [
        i for i, (loss, target_train) in enumerate(run_info["losses"]) if target_train
    ]
    fig, ax = plt.subplots(1, 1, figsize=(20, 10))
    ax.plot(
    ...
    ax.scatter(
        ...
        [
            loss
            for i, (loss, _) in enumerate(run_info["losses"])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--resume-from",
        default="",
        help="Resume from the latest model in the given folder",
    )
    ...
    parser.add_argument(
        ...
        default=1000,
        type=int,
        help="The amount of steps after which the target network is updated",
    )
    parser.add_argument(
        "-m",
        "--update-main-network-every",
        default=32,
        type=int,
        help="The amount of steps after which the main network is updated",
    )
    parser.add_argument(
        "--epsilon",
        default=1.0,
        type=float,
        help="The starting epsilon parameter for the epsilon-greedy policy",
    )
    parser.add_argument(
        "-d",
        "--decay-factor",
        default=0.1,
        type=float,
        help="The speed at which the epsilon factor decreases",
    )
    ...
    parser.add_argument(
        ...
        default=350,
        type=int,
        help="How many games to play during training",
    )
    ...
    parser.add_argument(
        ...
        default=32,
        type=int,
        help="The hidden size of the neural network",
    )
    parser.add_argument(
        "--conv",
        default=False,
        action="store_true",
        help="Uses a convolutional NN as brain",
    )
    parser.add_argument(
        "--random-seed",
        default=0,
        type=int,
        help="The random seed to initialize all random number generators",
    )
    ...
    parser.add_argument(
        ...
        default=32,
        type=int,
        help="The size of the mini-batches to train the main network on",
    )
    parser.add_argument(
        "-e",
        "--epochs",
        default=10,
        type=int,
        help="How many times the model will go through the same sample of states in a single training session",
    )
    parser.add_argument(
        "--max-memory",
        default=10_000,
    ...
    parser.add_argument(
        ...
        default=0.7,
        type=float,
        help="The learning rate for the Bellman equation",
    )
    parser.add_argument(
        "--discount-factor",
        default=0.618,
        type=float,
        help="The discount factor for the Bellman equation",
    )
    parser.add_argument(
        "--min-replay-size",
        default=1000,
        type=int,
        help="Minimum amount of samples to trigger training",
    )
    parser.add_argument(
        "--n-samples-to-train-on",
        default=10_000,
        type=int,
        help="Samples used for every training step",
    )
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Prints a message every time a training event happens",
    )
    parser.add_argument(
        "--store-run-at",
        default="model_checkpoints",
        help="Where to store the training runs",
    )
    parser.add_argument(
        "--no-store",
        default=False,
        action="store_true",
        help="Don't store training parameters and trained model",
    )
    parser.add_argument(
        "--checkpoint-every",
        default=100,
        type=int,
        help="How often (# episodes) to store a checkpoint model",
    )
    parser.add_argument(
        "--cuda",
